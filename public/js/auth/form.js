...
function verifyUserid() {
	...
	if(userid === '' || userid.length < 6 || userid.length > 16) {
		...
		// axios
		// .get('/api/auth/verify', { params : { key: 'userid', value: userid }, function() })
		// .then(function (r) {
...
function verifyPasswd() {
	...
	else return verifyTrue(passwdEl, passwdTxt)
}
...
function verifyPasswd2() {
	...
	else return verifyTrue(passwd2El, passwd2Txt)
}
...
function verifyPasswdEqual() {
	...
	if(passwd !== passwd2) {
		...
		verifyTrue(passwd, passwdTxt);
		verifyTrue(passwd2, passwd2Txt);
		return true;
	}
}
...
function verifyUsername() {
	...
	else return verifyTrue(usernameEl, usernameTxt)
}
